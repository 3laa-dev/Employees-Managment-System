...
const getAllEmployees = asyncWrapper(async (req, res, next) => {
    ...
    if (manager) {
        ...
        sendData(res, status.SUCCESS, { Employees: Employees });
    }
    if (admin) {
...
const getEmployee = asyncWrapper(async (req, res, next) => {
    ...
    if (admin) {
        const employee = await Employee.findById(ID, { __v: false });
        sendData(res, status.SUCCESS, { Employee: employee });
    }
    ...
    if (bool) {
        ...
        if (departmentId) {
            ...
            if (departmentId != employee.departmentId) {
                department.numberOfEmployees++;
                const department2 = await Department.findById(employee.departmentId);
                department2.numberOfEmployees--;
        ...
        if (updatedEmployee)
            sendData(res, status.SUCCESS, { updatedEmployee: updatedEmployee })
        else {
            const error = new appError("this employee is not exist");
            next(error);
        }
    }
    const error = new appError("only admin and mangers can update employees data")
    next(error);
})
